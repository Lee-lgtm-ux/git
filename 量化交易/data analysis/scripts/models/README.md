...
#### 1. 当前财务质量分：`quality_score`

`quality_score` 衡量的是公司当前这一期财务质量在同一财报季度所有股票里的相对位置。

计算方式：

1. 对每个财务指标，在同一个 `statDate` 横截面内做百分位排名。
2. 对正向指标，数值越高百分位越高。
3. 对反向指标，先取反再排名，所以数值越低百分位越高。
4. 把可用指标的百分位取平均，得到 `quality_score`。
...
所以 `quality_score = 80` 的意思不是“绝对分数 80 分”，而是“这家公司当前财务质量大约处在同季度股票的较高分位”。

#### 2. 预测质量改善：`predicted_quality_trend`
...
```text
predicted_quality_trend = 预测未来 4 个季度 quality_score 会改善多少
```
...
```text
当前 quality_score = 55
predicted_quality_trend = +12
predicted_future_quality_score = 67
```

这表示模型认为这家公司未来 4 个季度的财务质量分可能从 55 改善到 67。

#### 3. 模型排序分：`score` / `trend_score`

模型输出的预测值会再做一次横截面排名：

```text
score = prediction 的百分位排名 * 100
trend_score = predicted_quality_trend 的百分位排名 * 100
```

也就是说：

- `score` 越高，表示模型越看好它在股票池中的相对排名。
- `trend_score` 越高，表示模型认为它的财务质量改善幅度越靠前。
...
```text
先看当前质量分 quality_score
再看未来改善 predicted_quality_trend
最后看横截面排名 score / rating
```

重要提醒：这个评级表示“基本面质量改善潜力”，不是短期买卖信号，也不是未来股价收益承诺。
